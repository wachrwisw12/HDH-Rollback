...
SELECT pt.hn,
       ps.person_id,
       ps.cid,
       ps.pname || ps.fname || ' ' || ps.lname AS full_name,
       CONCAT(
           TRIM(COALESCE(pt.addrpart, '')),
           ' หมู่ ',
           TRIM(COALESCE(pt.moopart, '')),
           ' ',
           TRIM(COALESCE(td.full_name, ''))
       ) AS full_address
FROM person ps
LEFT JOIN patient pt ON ps.cid = pt.cid
LEFT JOIN thaiaddress td 
    ON td.addressid = CONCAT(
        TRIM(COALESCE(pt.chwpart::text, '')),
        LPAD(TRIM(COALESCE(pt.tmbpart::text, '')), 2, '0'),
        LPAD(TRIM(COALESCE(pt.moopart::text, '')), 2, '0')
    )
WHERE ps.person_id IN (` + strings.Join(placeholders, ",") + `)
	...
	args2 := make([]interface{}, len(missing))
	...
	SELECT pt.hn,
	       ...
	       ps.cid,
	       ps.pname || ps.fname || ' ' || ps.lname AS full_name,
	       pt.addrpart || ' หมู่ ' || pt.moopart || ' ' || td.full_name AS address_name
	FROM person ps
	LEFT JOIN patient pt ON ps.cid = pt.cid
	...
	rows2, err := h.db.Query(query2, args2...)
	if err != nil {
		return nil, err
	}
	defer rows2.Close()
